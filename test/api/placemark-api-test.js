...
suite("Placemark API tests", () => {
  ...
  setup(async () => {
    ...
    for (let i = 0; i < testPlacemarks.length; i += 1) {
      ...
      createdPlacemarks[i] = await placemarkService.createPlacemark(
        collection._id,
        testPlacemarks[i]
      );
    }
  ...
  test("create a placemark", async () => {
    const newPlacemark = await placemarkService.createPlacemark(
      collection._id,
      testPlacemarks[0]
    );
  ...
  test("get one placemark - success", async () => {
    const returnedPlacemark = await placemarkService.getPlacemark(createdPlacemarks[0]._id);
